...
float map(vec3 p) {
    p *= 3.0;
    float thickness = 0.28 * u_beatPulse + 0.12 * u_bassBody; // walls swell on the pulse
    return abs(dot(sin(p*4)+4, cos(p.yzx))) / 4.0 - thickness;
}
...
void main() {
    ...
    vec3 ro = vec3(0.0, 0.0, u_time * 0.6);       // fly forward
    vec3 rd = normalize(vec3(uv, 1.2 - 0.3 * u_beatPulse));  // pulse punches the camera in

    float t = 0.0;
    for (int i = 0; i < 80; ++i) {
        float d = map(ro + rd * t);
        if (d < 0.001 || t > 30.0) break;
        t += d * 0.4;                              // 0.7: gyroid SDF safety factor
    }

    vec3 p = ro + rd * t;
    float diff = clamp(dot(normal(p), -rd), 0.1, 1.0);   // headlight
    // Black -> purple: deep violet in shadow, lilac toward the light.
    vec3 base = mix(vec3(0.18, 0.0, 0.40), vec3(0.70, 0.25, 1.0), 0.5 + 0.5 * sin(t * 10.4 + 2.0 * u_airPresence));
    vec3 col = base * diff ;             // shade (black where unlit)
    col *= 1 + 0.8 * u_energyLevel;    // energy drives brightness

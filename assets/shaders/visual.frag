...
void main(){
	...
	float speed = mix(0.1, 1.5, clamp(u_bandHigh + 0.3*u_onset, 0.0, 1.0));
	float baseF = mix(18.0, 80.0, clamp(u_bandMid, 0.0, 1.0));
	float detune = 0.02 + 0.18 * clamp(u_rms, 0.0, 1.0);
	float wobble = 0.6 * u_bandLow;
	...
	vec2 pw = uv;
	float r = length(uv);
	float warp = 0.02 * sin(6.2831*r + t*0.8) * (0.5 + 0.5*u_bandHigh);
	...
	m = pow(m, mix(1.2, 0.6, clamp(u_onset*1.5, 0.0, 1.0)));
	...
	float bias = 0.08 + 0.25*u_rms + 0.35*u_onset;
